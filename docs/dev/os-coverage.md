...
| Windows | Unit suite green in CI since 2026-08-17. The counts below are from that run and are NOT re-measured per release — the suite has grown since (642 in the 1.2.0 export). Some tests skip themselves with a printed reason. The init path itself is still paper-reviewed — VERIFY ON FIRST REAL DEPLOYMENT |
...
and fix whatever setup instructions were wrong, so the next machine benefits.
