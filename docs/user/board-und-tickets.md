...
date: 2026-08-15
tags: [docs, feature]
summary: Wie man das Kanban-Board öffnet, Tickets anlegt und sie von Agenten abarbeiten lässt; Einrichtung per „init".
---
...
Tippe einfach **init** in den Chat der Werkbank. Der Assistent führt dich dann
durch die Einrichtung: Er fragt, welches Projekt deine Tickets standardmäßig
betreffen sollen, erklärt dir, wie sich eine Projekt-Session ihr Ticket selbst
vom Board zieht („zieh dir dein Ticket"), und bietet dir an, den dafür nötigen
Skill zu installieren. Du kannst „init" jederzeit erneut tippen, etwa um das
Standard-Projekt zu wechseln.

## Das Board öffnen

Das Board **startet automatisch**, sobald du dich am Rechner anmeldest, und
kommt nach einem Absturz von selbst wieder hoch — öffne einfach dein
Lesezeichen: <http://127.0.0.1:8765>. Es läuft nur auf diesem Rechner, niemand
sonst kann es sehen oder bedienen (das bleibt so, solange du es nicht
ausdrücklich anders entscheidest). Sollte es doch einmal nicht erreichbar
sein, sag dem Assistenten: **„Starte das Board."**

Das Board hat fünf Spalten:
...
Beide Skills — „zieh dir dein Ticket" und das Bug-Melden — sind auf deinem
Rechner installiert und stehen in jeder Projekt-Unterhaltung zur Verfügung.
...
- Jedes Ticket ist eine einfache Textdatei im Ordner `tickets/` — nichts kann in
  einer Datenbank verloren gehen, und jede Änderung wird in git gesichert.
- Löschen geht im Detailfenster (roter Knopf, mit Rückfrage). Weil alles in git
  gesichert ist, kann der Assistent ein gelöschtes Ticket auf Zuruf
  wiederherstellen.
